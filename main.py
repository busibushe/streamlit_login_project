...
def load_data(file):
    ...
    try:
        ...
        if 'Sales Date' not in df.columns: raise ValueError("Kolom 'Sales Date' atau 'Date' tidak ditemukan.")
        if 'Branch' not in df.columns: raise ValueError("Kolom 'Branch' tidak ditemukan.")
        ...
        df.dropna(subset=numeric_cols + ['Sales Date', 'Branch'], inplace=True)
        return df
...
def load_qc_data(file):
    ...
    try:
        ...
        required_cols = ['Branch', 'Month', 'QC Score']
        if not all(col in df.columns for col in required_cols):
            raise ValueError(f"File QC harus memiliki kolom: {', '.join(required_cols)}")
            
        # Ubah kolom 'Month' menjadi format periode bulanan yang konsisten
        df['Bulan'] = pd.to_datetime(df['Month'], errors='coerce').dt.to_period('M')
...
def display_analysis_with_details(title, analysis_text, p_value):
    ...
    if p_value is not None:
        with st.expander("Lihat detail signifikansi (p-value)"):
            ...
            if p_value < 0.05: st.success("✔️ **Kesimpulan:** Tren ini **nyata secara statistik**.")
            else: st.warning("⚠️ **Kesimpulan:** Tren ini **tidak signifikan secara statistik**.")
    st.markdown("---")
...
# --- Unggah File ---
sales_file = st.sidebar.file_uploader("1. Unggah File Penjualan Anda", type=["xlsx", "xls", "csv"])
qc_file = st.sidebar.file_uploader("2. (Opsional) Unggah File QC Score Anda", type=["xlsx", "xls", "csv"])

if sales_file is None:
    st.info("Selamat datang! Silakan unggah file data penjualan Anda untuk memulai.")
    st.stop()
...
min_date = df_sales['Sales Date'].min()
max_date = df_sales['Sales Date'].max()
date_range = st.sidebar.date_input("Pilih Rentang Tanggal", value=(min_date, max_date), min_value=min_date, max_value=max_date)

if len(date_range) != 2: st.warning("Mohon pilih rentang tanggal yang valid."); st.stop()
start_date, end_date = date_range

# --- Filter & Agregasi Data ---
df_filtered = df_sales[(df_sales['Branch'] == selected_branch) & (df_sales['Sales Date'] >= start_date) & (df_sales['Sales Date'] <= end_date)]
...
if not monthly_agg.empty:
    ...
    days_in_month = monthly_df.groupby('Bulan')['Sales Date'].nunique()
    monthly_agg = pd.merge(monthly_agg, days_in_month, on='Bulan')
    monthly_agg.rename(columns={'Sales Date': 'JumlahHariPenjualan'}, inplace=True)
    monthly_agg['AvgDailySales'] = monthly_agg['TotalMonthlySales'] / monthly_agg['JumlahHariPenjualan']
    ...
    if df_qc is not None:
        ...
        # Gabungkan (merge) berdasarkan bulan. Gunakan 'left' join agar semua bulan penjualan tetap ada.
...
# Tampilan KPI
# ... (Kode KPI sama seperti sebelumnya) ...
col1, col2, col3 = st.columns(3)
if len(monthly_agg) >= 2:
    last_month = monthly_agg.iloc[-1]; prev_month = monthly_agg.iloc[-2]
    sales_delta = (last_month['AvgDailySales'] - prev_month['AvgDailySales']) / prev_month['AvgDailySales']
    ...
    col1.metric("💰 Rata-Rata Penjualan Harian", f"Rp {last_month['AvgDailySales']:,.0f}", f"{sales_delta:.1%}")
    col2.metric("🛒 Total Transaksi Bulanan", f"{last_month['TotalTransactions']:,}", f"{trx_delta:.1%}")
    col3.metric("💳 AOV Bulanan", f"Rp {last_month['AOV']:,.0f}", f"{aov_delta:.1%}")
elif not monthly_agg.empty:
    last_month = monthly_agg.iloc[-1]
    col1.metric("💰 Rata-Rata Penjualan Harian", f"Rp {last_month['AvgDailySales']:,.0f}")
    col2.metric("🛒 Total Transaksi Bulanan", f"{last_month['TotalTransactions']:,}")
    col3.metric("💳 AOV Bulanan", f"Rp {last_month['AOV']:,.0f}")
st.markdown("---")
...
with col_chart:
    if not monthly_agg.empty:
        # Grafik Rata-Rata Penjualan Harian (diganti dari total penjualan)
        ...
        # Grafik Transaksi & AOV (tidak berubah)
        ...
        if df_qc is not None and 'QC Score' in monthly_agg.columns:
            st.subheader("Tren QC Score Bulanan")
            # .dropna() digunakan agar garis tidak terputus jika ada bulan tanpa data QC
...
with col_table:
    # Tabel Menu Terlaris (tidak berubah)
